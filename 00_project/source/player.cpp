...
CPlayer::EMotion CPlayer::UpdateMove(void)
{
	...
	{ // デッドゾーン以上の場合
		...
		// 変数を宣言
		float fMove = fLTilt * 0.0001f;	// プレイヤー移動量

		// 移動量を更新
		m_move.x += sinf(pPad->GetPressLStickRot() + pCamera->GetRotation().y + HALF_PI) * fMove * 1.5f;
		m_move.z += cosf(pPad->GetPressLStickRot() + pCamera->GetRotation().y + HALF_PI) * fMove * 1.5f;
