...
namespace
{
	...
	namespace fire
	{
		...
		const float	POSGAP		= 10.0f;	// 炎の位置ずれ量
		const float	MOVE		= -1.0f;	// 炎の移動量
		const int	SPAWN		= 3;		// 炎の生成数
		const int	EFF_LIFE	= 5;		// 炎の寿命
		const float	INIT_RAD	= 80.0f;	// 炎の半径
		const float	INIT_SUBRAD	= 0.8f;		// 炎の半径の減算量
